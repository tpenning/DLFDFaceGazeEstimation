...
class FDGazeModelAlexNet(GazeModel):
    def __init__(self, model_name: str, input_channels: int, lc_hc: str, device=get_device()):
        ...
        self.channels = [96, 256, 384, 384, 256, 2304]
        if lc_hc == "HC":
            self.channels = [channel * 2 for channel in self.channels]

        # Convolutional layers changed to adapt to the small data size
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=self.input_channels, out_channels=self.channels[0], kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2))
        self.conv2 = nn.Sequential(
            nn.Conv2d(in_channels=self.channels[0], out_channels=self.channels[1], kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2))
